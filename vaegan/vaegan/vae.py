...
def sample_from_normal(dist_mean, dist_logvar):
    ...
    # 
    # To help us build the VAE, we need you to implement the sampling of a point from a normal distribution parameterized by a given mean and and 
    ...
    # ========================  
    eps = tf.random.normal(tf.shape(dist_mean))
    std = tf.exp(0.5*dist_logvar)
    sampledZ = dist_mean + eps*std
    return sampledZ
...
def kl_divergence(dist_mean, dist_logvar):
    ...
    kl_sample = tf.reduce_sum(kl, axis=1)
    divKL_AllSamples = tf.reduce_mean(kl_sample)
...
class VAE(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        z_mean, z_logvar = self.encoder(inputs, training=training) # extract the input distribution through encoder
        z = sample_from_normal(z_mean, z_logvar) # generate latent space from the sampled distritbuion
        recons = self.decoder(z, training=training) # reconstruct the data using the decoder
        return recons
...
class ConditionalVAE(VAE):
    ...
    def make_conditional_input(self, images, labels):
        ...
        labels_reshape = tf.reshape(labels, (n, 1, 1, self.n_classes))
        labels_fmt = tf.broadcast_to(labels_reshape, (n, w, h, self.n_classes))

        # 3. Finally since the tensors are the same dimension along the last axis, we can concatenate them together. 
        imagesConcatenatedWithPerPixelLabelVectors = tf.concat([images, labels_fmt], axis=-1)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            latent_labels_concat = tf.concat([z, class_real], axis=-1)

            # *** Step 4: Use decoder to reconstruct image from the concatenated result
            recons = self.decoder(latent_labels_concat, training=True)
